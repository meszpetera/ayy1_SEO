...
if (isset($_SESSION['pending_hibabejelentes'])) {
    foreach ($_SESSION['pending_hibabejelentes'] as $key => $value) {
        $variables[$key] = $value;
    }
}
unset($_SESSION['pending_hibabejelentes']);

$template->assign_var_array($variables);

$main_content = $template->compile("sys/lang/" . $lang . "/root_service");
