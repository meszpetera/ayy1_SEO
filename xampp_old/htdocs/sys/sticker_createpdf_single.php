  ...
  else
  {
    ...
    $mysql->execute($sql['setutf']);
      ...
      if($stmt->execute())
      {
        ...
        $pdf->Image('https://'.($_SERVER['HTTP_HOST']).'/newqr/?text='.urlencode('https://'.($_SERVER['HTTP_HOST']).'/sys/aktualis_ajax_truck_details.php?lang=hun&id='.($truckdetails[0]['truck_id']).'#'.($truckdetails[0]['truck_saxon-id'])),87,37,60,60,'png');
