...
if (isauth() && isset($_REQUEST['offerid'])) {
    ...
    $offer_pdf = get_pdf($_REQUEST['offerid']);
...
//    print($offer['offer_data']['offer_status']);
    if (($offer['offer_data']['offer_status'] == 4) || ($offer['offer_data']['offer_status'] == 1) || ($offer['offer_data']['offer_status'] == 2)) {
        ...
        if ("***" . $offer['offer_data']['offer_date-closed'] . "***" != "***0000-00-00 00:00:00***") {
            $offerID_Fancy = mb_substr($offer['offer_data']['offer_date-closed'], 2, 2) . str_pad($offer['offer_data']['offer_id'], 4, '0', STR_PAD_LEFT);
        } else {
...
//      $pdf->SetFont('Arial', '', 10);      $pdf->Write(5, iconv("UTF-8", "ISO-8859-2", ' (' . $offer['userdata']['users_phone'] . ', ' . $offer['userdata']['users_email'] . ')'));
        ...
        //$pdf->MultiCell(180, 7, iconv("UTF-8", "ISO-8859-2", $offer['offer_data']['deliverymethod']));
        $pdf->MultiCell(180, 7, utf8_decode($offer['offer_data']['deliverymethod']));
        ...
        foreach ($offer['trucks'] as $truck) {
            $offer_pdf_truck = get_pdf_truck($truck['offer_truck_id']);
            ...
            if ($offer_pdf == 1) {
                ...
                $pdf->MultiCell(70, 7, iconv("UTF-8", "ISO-8859-2", $truck['offer_truck_type']), 0, 'L');
            }
            ...
            if ($offer_pdf == 1) {
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_forks'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_forks'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_full-height'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_full-height'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_cabin-height'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_cabin-height'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_length'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_length'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_width'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_width'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_lifting-column-height'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_lifting-column-height'])), 0, 'L');
                $pdf->Cell(60, 7, iconv("UTF-8", "ISO-8859-2", $language['truck-details_comment'] . ':'));
                $pdf->MultiCell(60, 7, iconv("UTF-8", "ISO-8859-2", trim($offer_pdf_truck['truck_desc'])), 0, 'L');
            }
...
//ha kell email
        if (isset($_REQUEST['sendEmail'])) {
            ...
            if ($_REQUEST['sendEmail'] == 1) {
                if (isset($_REQUEST['saxon_chk'])) {
                    if (strlen($_REQUEST['SName']) > 0) {
                        $megylevel = true;
                        $email_to = $_REQUEST['SName'];
                        $ell+=1;
                        //megy a saxonnak levél
                    }
                }
                if (isset($_REQUEST['megrendelo_chk'])) {
                    if (strlen($_REQUEST['MName']) > 0) {
                        $megylevel = true;
                        $ell+=2;
                        if ($ell == 3)
                            $email_to.=', ' . $_REQUEST['MName'];
                        else {
                            $email_to.=$_REQUEST['MName'];
                        }
                        //csak ekkor állítom át a statust
                        $lezaras = true;
                        //megy a megrendelőnek a levél
                    }
                }
                if (isset($_REQUEST['tovabbi_chk'])) {
                    if (strlen($_REQUEST['TName']) > 0) {
                        $megylevel = true;
                        $ell+=3;
                        if ($ell == 6 or $ell == 5 or $ell == 4)
                            $email_to.=', ' . $_REQUEST['TName'];
                        else {
                            $email_to.=$_REQUEST['TName'];
                        }
                        //megy további címekre a levél
                    }
                }
            } else {
                $megylevel = true;
            ...
            //window.open('https://www.saxonrt.hu/sys/offerrequest_createpdf.php?offerid=2697');
