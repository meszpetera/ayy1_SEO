				...
				for ($i = 0; $i < count($result); $i++) {
						...
						if ($stmt->execute()) {
								...
								foreach ($trucks as $truck) {
										$worksheet->writeUrl($row, 0, 'https://saxonrt.hu/sys/aktualis_ajax_truck_details.php?lang=' . $_REQUEST['lang'] . '&id=' . $truck['truck_id'], $truck['truck_saxon-id']);
										...
										foreach ($trucks as $truck) {
											$item++;
												if ($truck['truck_saxon-id'] != '') {
														$worksheet->writeUrl($row, 0, 'https://saxonrt.hu/sys/aktualis_ajax_truck_details.php?lang=' . $_REQUEST['lang'] . '&id=' . $truck['truck_id'], $truck['truck_saxon-id'], $format_body_url);
												}
