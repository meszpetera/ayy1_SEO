...
$sql = array(
    ...
    "truck_details_public" => "SELECT
        ...
        `truck_product_status`,
        (SELECT COUNT(*) FROM truck_images WHERE truck_images.`image_truck-id` = {1}) AS truck_imagecount
      FROM trucks WHERE `trucks`.truck_state IN ('A', 'B') AND `trucks`.truck_public = 1 AND !( (`trucks`.`truck_depot`='3' OR `trucks`.`truck_depot`='0' ) AND `trucks`.`truck_sub-depot`='0') AND `trucks`.`truck_id` = '{1}'",
    ...
    "aktualis-filter_status_list" => "SELECT * FROM truck_status_[0] WHERE IF((SELECT count(*) FROM `trucks` WHERE `ID`=`truck_status` AND truck_public = 1)>0,1,0) ORDER BY value",
...
// WHERE `truck_type` = {1} AND `truck_ispart` = {2} AND `truck_state` = 'A'
    ...
    "excel_get-trucks_2date" => "SELECT
      ...
      CONCAT_WS(' ', (SELECT value FROM truck_location WHERE truck_location.depot = trucks.truck_depot AND truck_location.subdepot = 0), (SELECT value FROM truck_location WHERE truck_location.depot = trucks.truck_depot AND truck_location.subdepot = trucks.`truck_sub-depot` AND truck_location.subdepot != '0')) AS truck_location
    FROM `trucks`
    WHERE `truck_type` = {1} AND `truck_ispart` = {2} AND `truck_state` = 'A' AND `trucks`.truck_public = 1
    ORDER BY
    IF( `truck_ispart` =  0 , 0 , 1 ),
    IF( `truck_ispart` =  2 , 0 , 1 ),
    IF( `truck_ispart` =  5 , 0 , 1 ),
...
// WHERE truck_public = {3} AND `truck_type` = {1} AND `truck_ispart` = {2} AND `truck_state` = 'A'
    ...
    "excel_get-trucks_2_R" => "SELECT
      ...
      CONCAT_WS(' ', (SELECT value FROM truck_location WHERE truck_location.depot = trucks.truck_depot AND truck_location.subdepot = 0), (SELECT value FROM truck_location WHERE truck_location.depot = trucks.truck_depot AND truck_location.subdepot = trucks.`truck_sub-depot` AND truck_location.subdepot != '0')) AS truck_location
    FROM `trucks`
    WHERE truck_public = 1 AND `truck_saxon-id` LIKE \"R%\" AND `truck_type` = {1} AND `truck_ispart` = {2} AND `truck_state` = 'A'
    ORDER BY
    IF( `truck_ispart` =  0 , 0 , 1 ),
    IF( `truck_ispart` =  2 , 0 , 1 ),
    IF( `truck_ispart` =  5 , 0 , 1 ),
...
;",
    ...
    "promo:get" => "SELECT `truck_id` FROM `trucks` WHERE `truck_status` IN (1, 2, 11) AND (SELECT COUNT(*) FROM `truck_images` WHERE `truck_images`.`image_truck-id` = `trucks`.`truck_id`) > 0 AND `truck_cost` > 0 AND `truck_state` = 'A' AND `truck_public` = 1",
