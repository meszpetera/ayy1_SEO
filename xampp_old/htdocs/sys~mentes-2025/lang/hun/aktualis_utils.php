...
function get_filtered_list($akcios, $saxon_id, $make, $model, $fuel, $max_load, $status, $cost, $location, $offset, $maxshow, $type, $ispart = 0, $function, $terstatus=false) {
    ...
    if (!isauth()) {
        ...
        $query = "WHERE truck_public = 1 " .$qry;
    }
            ...
            if ($stmt->num_rows() > 0) {
                ...
                for ($i = 0; $i < count($result); $i++) {
                    ...
                    if ($img_exists == false) {
                        ...
                        $list[3] .= "<img src=\"$img_t\" style=\"float: left; margin:5px;margin-right: 8px; width: 50px; height: 50px;\" alt=\"" . htmlspecialchars($result[$i]['truck_type'] . ' ' . $result[$i]['truck_make']) . "\" title=\"" . htmlspecialchars($result[$i]['truck_type'] . ' ' . $result[$i]['truck_make']) . "\"/></a>";
                        $img_missing = false;
                    }
                    ...
                    if ($ispart == 1) {
                        $make = $result[$i]['truck_make'] . " " . iconv("ISO-8859-2", "UTF-8", iconv("UTF-8", "ISO-8859-2", $result[$i]['truck_fuel']));
                        if (strlen($make) > 33) {
                            $make = mb_substr($make, 0, 30);
                            $make .= "...";
                        }
                        $list[3] .= "<br />" . $make . "<br/>" . iconv("ISO-8859-2", "UTF-8", iconv("UTF-8", "ISO-8859-2", $result[$i]['truck_model']));
                    } else {
                        $make = $result[$i]['truck_make'] . " " . iconv("ISO-8859-2", "UTF-8", iconv("UTF-8", "ISO-8859-2", $result[$i]['truck_model']));
                        if (strlen($make) > 24) {
                            $make = mb_substr($make, 0, 21);
                            $make .= "...";
                        }
                        $list[3] .= "<br />" . $make . "<br/>" . iconv("ISO-8859-2", "UTF-8", iconv("UTF-8", "ISO-8859-2", $result[$i]['truck_fuel']));
                    }
        		...
        		// $list[3] = $sql['trucks_filteredlist'].' | '.$lang.' | '.$query.' | '.$limit;
        }
        else {
            $list = 0;
        }
       ...
       if ($mode==''){
          $result .= '
            ...
            switch ($mode){
              case 'beszer':
                   $ts1 = 'checked';
                   $di0=$di2=$di3=$di4=$di5 = 'disabled';
                   $bs0=$bs2=$bs3=$bs4=$bs5 = 'style="color: gray;"';
                   break;
              case 'eladas':
                   $ts0 = 'checked';
                   $di1=$di2=$di3=$di4=$di5 = 'disabled';
                   $bs1=$bs2=$bs3=$bs4=$bs5 = 'style="color: gray;"';
                   break;
              case 'rakmoz':
                   $ts0 = 'checked';
                   $di1=$di2=$di3=$di4=$di5 = 'disabled';
