...
if (!loggedin() || !isauth()) {
    ...
    foreach ($_REQUEST as $key => $value) {
        if (mb_substr($key, 0, 4) == 'make') {
            ...
            if ($value == "") {
                //delete
                $stmt = $mysql->prepare($sql['admin_edit_function:delete']);
                ...
                if ($key[0] == 'X') {
                    //insert new
                    if (!isset($IDs[$key])) {
                        $stmt = $mysql->prepare($sql['admin_edit_function:insertNoID']);
                        ...
                        $stmt = $mysql->prepare($sql['admin_edit_function:insert']);
                    ...
                    //update or insert
                    $stmt = $mysql->prepare($sql['admin_edit_function:query']);
                    $stmt->bind_params($lng, $key);
                    if ($stmt->execute()) {
                        $typelist = $stmt->fetch_all();
                        if ($typelist[0]['result'] == 1) {
                            //update
                            $stmt = $mysql->prepare($sql['admin_edit_function:update']);
...
//                                print('updating ' . $lng . '::' . $key . ' to ' . $value . '<br />');
    ...
    /*
      ...
      {
      $stmt = $mysql->prepare($sql['admin_edit_function:update_ispart_enable']);
      $stmt->bind_params($value, $toenable);
      $stmt->execute();
      $stmt = $mysql->prepare($sql['admin_edit_function:update_ispart_disable']);
      $stmt->bind_params($value, $toenable);
      $stmt->execute();
      } */

    redirect_in_site("?page=admin_edit_functions&lang=$lang");
}
?>