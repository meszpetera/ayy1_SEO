...
if (isauth()) {
    ...
    if (isset($_REQUEST['ID']) and $_REQUEST['ID']>0) {
        $muvelet=true;
        $res = update_delivery($_REQUEST['ID'], $_REQUEST['Name']);
        if ($res > 0) {
            $data = get_delivery_id($_REQUEST['ID']);
            $variables = array(
                "NAME" => $data['Name'],
                "ID" => $data['ID'],
    ...
    if ((!isset($_REQUEST['ID']) or (int)$_REQUEST['ID']==0) and isset($_REQUEST['Name']) ) {
        ...
        $data = get_text_delivery($_REQUEST['Name']);
        if (empty($data)) {
            //insert mehet
            $res = insert_delivery($_REQUEST['Name']);
            if ($res > 0) {
                $data = get_delivery_id($res);
                $variables = array(
                    "NAME" => $data['Name'],
                    "ID" => $data['ID'],
                    "SUCCES" => "A szerkesztés sikeres.",
            ...
            //update
            $res = update_delivery($data['ID'], $_REQUEST['Name']);
            if ($res > 0) {
                $data = get_delivery_id($data['ID']);
                $variables = array(
                    "NAME" => $data['Name'],
                    "ID" => $data['ID'],
                    "SUCCES" => "A szerkesztés sikeres.",
    ...
    if ($muvelet) {
        ...
        $main_content = $template->compile("lang/hun/edit_delivery");
        ...
        if (isset($_REQUEST['tip'])) {
            if ($_REQUEST['tip'] == 1) {
                ...
                if (!isset($_REQUEST['param'])) {
                    ...
                    //kiszedem az id-t
                     $data = get_delivery_id($_REQUEST['param']);
                    //print_r($data);
                    $variables = array(
                        "NAME" => $data['Name'],
                        "ID" => $data['ID'],
