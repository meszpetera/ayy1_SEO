...
function get_filter_list($fieldname, $ispart = 0) {
    ...
    if ($fieldname == "make") {
        ...
        $stmt = $mysql->prepare($sql['aktualis-filter_status']);
        $stmt->bind_params($lang);
        ...
        */
        $result[0]['ID'] = "0-1300";
        $result[0]['value'] = "0-1300 kg";

        $result[1]['ID'] = "1000-1999";
        $result[1]['value'] = "1000-1999 kg";

        $result[2]['ID'] = "2000-2999";
        $result[2]['value'] = "2000-2999 kg";

        $result[3]['ID'] = "3000-3499";
        $result[3]['value'] = "3000-3499 kg";

        $result[4]['ID'] = "3500-4500";
        $result[4]['value'] = "3500-4500 kg";

        $result[5]['ID'] = "3500-4500";
        $result[5]['value'] = "3500-4500 kg";

        $result[6]['ID'] = "4000-5000";
        $result[6]['value'] = "4000-5000 kg";

        $result[7]['ID'] = "5000-6000";
        $result[7]['value'] = "5000-6000 kg";

        $result[8]['ID'] = "6000-9000";
        $result[8]['value'] = "6000-9000 kg";

        $result[9]['ID'] = "9000-12000";
        $result[9]['value'] = "9000-12000 kg";

        $result[9]['ID'] = "12000-9999999";
        $result[9]['value'] = "12000 kg ". $language['above'];
        ...
        $result[0]['ID'] = "1-1999";
        $result[0]['value'] = "1-1999 &euro;";
        $result[1]['ID'] = "2000-3999";
        $result[1]['value'] = "2000-3999 &euro;";
        $result[2]['ID'] = "4000-5999";
        $result[2]['value'] = "4000-5999 &euro;";
        $result[3]['ID'] = "6000-7999";
        $result[3]['value'] = "6000-7999 &euro;";
        $result[4]['ID'] = "8000-9999";
        $result[4]['value'] = "8000-9999 &euro;";
        $result[5]['ID'] = "10000-11999";
        $result[5]['value'] = "10000-11999 &euro;";
        $result[6]['ID'] = "12000-15999";
        $result[6]['value'] = "12000-15999 &euro;";
        $result[7]['ID'] = "16000-19999";
        $result[7]['value'] = "16000-19999 &euro;";
        $result[7]['ID'] = "20000-500000";
        $result[7]['value'] = "20000 &euro; " . $language['above'];
...
///átír
function get_filtered_list($akcios, $saxon_id, $make, $model, $fuel, $max_load, $status, $cost, $location, $offset, $maxshow, $type, $ispart = 0, $function) {
    global $sql;
    global $lang;
    global $language;
    //if ($ispart==2) echo $ispart.'---';
    $query = "WHERE truck_public = 1 ";
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->num_rows() > 0) {
                ...
                for ($i = 0; $i < count($result); $i++) {
                    ...
                    $list[3] .= "<div id=\"$id\" class=\"$class\">";  //onselect attr. kiv�ve, FARM, 08-08-14
                    if ($img_exists == false) {
                        $list[3] .= "<img src=\"$img\" style=\"float: left; margin:5px;margin-right: 8px; width: 50px; height: 50px;\" alt=\"".htmlspecialchars($result[$i]['truck_type'].' '.$result[$i]['truck_make'].' '.$result[$i]['truck_model'])."\" title=\"".htmlspecialchars($result[$i]['truck_type'].' '.$result[$i]['truck_make'].' '.$result[$i]['truck_model'])."\"/>";
                    } else {
                        $list[3] .= "<a href=\"img/trucks/$img\" class=\"highslide\" onclick=\"return hs.expand(this);\" border=\"0\">";
                        $list[3] .= "<img src=\"$img_t\" style=\"float: left; margin:5px;margin-right: 8px; width: 50px; height: 50px;\" alt=\"".htmlspecialchars($result[$i]['truck_type'].' '.$result[$i]['truck_make'].' '.$result[$i]['truck_model'])."\" title=\"".htmlspecialchars($result[$i]['truck_type'].' '.$result[$i]['truck_make'].' '.$result[$i]['truck_model'])."\"/></a>";
                    }
                    $list[3] .= "<table style=\"position:relative;top:2px;line-height:1.4;height:50px;border-collapse:collapse;\">";
                    $list[3] .= "<tr><td style=\"width:247px;border-right:1px solid #8fbcb9;\"><strong>" . $result[$i]['truck_saxon-id'] . "</strong>&nbsp;&nbsp;";

                    if (isauth()) {
                        $list[3] .= "<a class=\"detail_btn\" href=\"sys/aktualis_ajax_truck_details.php?lang=$lang&amp;id=" . $result[$i]['truck_id'] . "\" onclick=\"return hs.htmlExpand(this, {align: 'center', objectType: 'iframe', width: 780, height: 700});\">" . $language['details'] . "</a>";
                    }

                    if ($ispart == 1) {
                        $make = $result[$i]['truck_make'] . " " . $result[$i]['truck_fuel'];
                        if (strlen($make) > 33) {
                            $make = substr($make, 0, 30);
                            $make .= "...";
                        }
                    ...
                    $list[3] .= "</td><td style=\"padding:5px;width:250px;border-right:1px solid #8fbcb9;\">" . $result[$i]['truck_type'] . "<br/>" . $result[$i]['truck_fuel'] . "<br />" . $result[$i]['truck_status'];
                    $list[3] .= "<br/></td>";
                    $list[3] .= "<td style=\"width:210px;padding:5px;\">";
                    ...
                    if (isauth()) {
                        ...
                        $list[3] .= '<a href="sys/admin_aktualis_truck_menu.php?id=' . $result[$i]['truck_id'] . '" class="highslide" onclick="return hs.htmlExpand(this, {objectType: \'iframe\', width: 260, height: 260, preserveContent: false });" border="0">m&#369;veletek</a><br /> ';
                    } else {
                        $list[3] .= "<a class=\"detail_btn\" href=\"sys/aktualis_ajax_truck_details.php?lang=$lang&amp;id=" . $result[$i]['truck_id'] . "\" onclick=\"return hs.htmlExpand(this, {align: 'center', objectType: 'iframe', width: 780, height: 780});\">" . $language['details'] . "</a>";
                    }

                    $list[3] .= "</td><td style=\"width:90px;text-align:right;padding:5px;\"><strong>" . (special_offer_active($result[$i]) ? '<span style="text-decoration: line-through;">' . $result[$i]['truck_cost'] . ' &euro;</span><br /><span style="color:#f00">' . $result[$i]['truck_special-offer-price'] . ' &euro;</span>' : $result[$i]['truck_cost'] . " &euro;") . "</strong><br />";
...
function generate_page_links($count, $maxshow, $offset) {
    ...
    $cont = false;
		...
		if($_REQUEST['lang']) { $_req[] = 'lang='.$_REQUEST['lang']; } else { $_req[] = 'lang=hun'; }
		if($_REQUEST['page']) { $_req[] = 'page='.$_REQUEST['page']; } else { $_req[] = 'page=aktualis'; }
		if($_REQUEST['ispart']) { $_req[] = '__ispart='.$_REQUEST['ispart']; }
		elseif($_REQUEST['__ispart']) { $_req[] = '__ispart='.$_REQUEST['__ispart']; }
		if($_REQUEST['cost']) { $_req[] = '__cost='.$_REQUEST['cost']; }
		elseif($_REQUEST['__cost']) { $_req[] = '__cost='.$_REQUEST['__cost']; }
		if($_REQUEST['location']) { $_req[] = '__location='.$_REQUEST['location']; }
		elseif($_REQUEST['__location']) { $_req[] = '__location='.$_REQUEST['__location']; }
		if($_REQUEST['status']) { $_req[] = '__status='.$_REQUEST['status']; }
		elseif($_REQUEST['__status']) { $_req[] = '__status='.$_REQUEST['__status']; }
		if($_REQUEST['maxload']) { $_req[] = '__maxload='.$_REQUEST['maxload']; }
		elseif($_REQUEST['__maxload']) { $_req[] = '__maxload='.$_REQUEST['__maxload']; }
		if($_REQUEST['type']) { $_req[] = '__type='.$_REQUEST['type']; }
		elseif($_REQUEST['__type']) { $_req[] = '__type='.$_REQUEST['__type']; }
		if($_REQUEST['fuel']) { $_req[] = '__fuel='.$_REQUEST['fuel']; }
		elseif($_REQUEST['__fuel']) { $_req[] = '__fuel='.$_REQUEST['__fuel']; }
		if($_REQUEST['function']) { $_req[] = '__function='.$_REQUEST['function']; }
		elseif($_REQUEST['__function']) { $_req[] = '__function='.$_REQUEST['__function']; }
		if($_REQUEST['make']) { $_req[] = '__make='.$_REQUEST['make']; }
		elseif($_REQUEST['__make']) { $_req[] = '__make='.$_REQUEST['__make']; }
		
        if($_REQUEST['saxonid']) { $_req[] = '__saxonid='.$_REQUEST['saxonid']; }
		elseif($_REQUEST['__saxonid']) { $_req[] = '__saxonid='.$_REQUEST['__saxonid']; }
		
        if($_REQUEST['akcios']) { $_req[] = '__akcios='.$_REQUEST['akcios']; }
		elseif($_REQUEST['__akcios']) { $_req[] = '__akcios='.$_REQUEST['__akcios']; }
		
        if($_REQUEST['saxonsearch']) { $_req[] = '__saxonsearch='.$_REQUEST['saxonsearch']; }
		elseif($_REQUEST['__saxonsearch']) { $_req[] = '__saxonsearch='.$_REQUEST['__saxonsearch']; }
...
function get_simple_search() {
    ...
    $result .= "<td colspan=4 style=\"text-align:center;\">" . $language['quicksearch'] . "<br /><input class=\"iLoginInput\" onkeyup=\"doMask();\" onkeypress=\"filters_changed(1)\" type=\"text\" id=\"saxonsearch\" style=\"margin:5px;margin-top:0px;margin-bottom:0px;width:180px;\" /></td>";
    $result .= "</tr>";
