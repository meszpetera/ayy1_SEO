...
function generate_minmax_simple_search($filterlist, $name, $default, $width, $td, $defaultvalue = '', $ext='') {
    ...
    $sellist .= "<input aria-label=\"\" aria-hidden=\"true\" style=\"visibility: hidden; display:none; position: absolute; z-index: -1;\" type=\"text\" onchange=\"filters_changed(1)\" id=\"{$name}\" value=\"{$defaultvalue}\" old=\"{$defaultvalue}\" />\n";
    ...
    $sellist .= "<input aria-label=\"-tól\" type=\"number\" onchange=\"{$onchange}\" onkeyup=\"{$onchange}\" id=\"{$name}-min\" value=\"{$curMin}\" min=\"{$min}\" max=\"{$max}\" step=\"{$stp}\" style=\"width:50%;\" /> -\n";
    $sellist .= "<input aria-label=\"-ig\" type=\"number\" onchange=\"{$onchange}\" onkeyup=\"{$onchange}\" id=\"{$name}-max\" value=\"{$curMax}\" min=\"{$min}\" max=\"{$max}\" step=\"{$stp}\" style=\"width:50%;\" /> ".(($ext)?("<span style=\"min-width: 3ch;\">{$ext}</span>"):(""))."\n";
    $sellist .= "</div>\n";
