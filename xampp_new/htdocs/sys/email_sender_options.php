...
if (isauth()) {
    ...
    if ($_REQUEST['offerid'] > 0) {
        $data = get_offer_useremail($_REQUEST['offerid']);
 
        $id=$_REQUEST['offerid'];
        $variables = array(
            "MNAME" => $data['users_email'],
            "SNAME" => "ajanlat@saxonrt.hu, saxonrt@t-online.hu",
            "ID" => $id,
            "PCIM" => "Címzettek megadása"
        );
        $template->assign_var_array($variables);

        $main_content = $template->compile("lang/hun/email_sender_options");
