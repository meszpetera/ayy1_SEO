...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<?php echo ($lang === 'hun') ? 'hu' : (($lang === 'eng') ? 'en' : 'hu'); ?>">
<head>
...
<div class="dMain">
  <div class="dContainer" id="MainContainer" role="banner">
    <div class="dHeader_<?php echo $lang ?>">
		<a href="https://<?php print($_SERVER['HTTP_HOST']); ?>" class="logo" aria-label="Saxon - Főoldal"></a>
...
<!-- <div style="position:absolute;top:55px;left:30px;font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 10px;font-weight:bold;color:#e5b802;"><?php echo $beta; ?></div>-->
		...
		<div class="dUserBox" id="UserBox" style="width:320px;height:40px;top:18px;right:20px;">
			<div class="dUserInfo" style="font-size: 16px;">
				<?php echo $user_box_top; ?>
			</div>
		</div>
		<div class="dUserBox" id="UserBox" style="width:310px;height:28px;top:60px;right:10px;">
			<div class="dUserInfo">
				<?php echo
				...
				?>
				<div class="top_keres" style="display: flex; align-items: center; gap: 5px;">
				<form action="?lang=<?php echo $lang; ?>&page=aktualis" method="post" class="TopSearch" >
					<input aria-label="Keresés Saxon számra" type="text" name="saxonsearch" id="saxonsearchtop" placeholder="<?php print($language['search_placeholder']) ?>" value="<?php echo $_REQUEST['saxonsearch'] ?>" class="TopSearchInput" style="border:0px solid #FFFFFF;width:152px;" />
					<script type="text/javascript">
      ...
      <div class="dMenu" role="navigation">
        <?php echo $main_menu; ?>
      </div>
    </div>
...
<div class="upTextScrollParent">
    ...
    <div class="dData" id="data" role="main">
      <?php echo $main_content; ?>
    </div>
    <div class="dTextScrollBottom" id="dTextScrollBottom" style="font-size: 12px;" role="contentinfo">
				<div style="position:absolute;right:-65px;top:-5px;"> <?php echo $user_box_bottom; ?> </div>
				<a href="/qr/?text=<?php print(urlencode('https://'.($_SERVER['HTTP_HOST']).($_SERVER['REQUEST_URI']).'')); ?>" class="highslide" onclick="return hs.expand(this,{'align':'center'});" target="_blank" style="float:left;"><img src="/qr/?text=<?php print(urlencode('https://'.($_SERVER['HTTP_HOST']).($_SERVER['REQUEST_URI']).'')); ?>" alt="Weboldal QR kódja" height="60" border="0" /></a>
				<a href="https://plusone.google.com/_/+1/confirm?hl=hu&url=<?php print(urlencode('https://'.($_SERVER['HTTP_HOST']).($_SERVER['REQUEST_URI']).'')); ?>&title=<?php print(urlencode($_title)); ?>" target="_blank" onclick="window.open(this.href, '_blank', 'status=0, height=300, width=550, resizable=0' ); return false;" style="float:left;margin-left:10px;"><img src="/pic/google.png" alt="Megosztás Google pluszon" height="60" border="0" /></a>
				<a href="https://www.facebook.com/sharer/sharer.php?u=<?php print(urlencode('https://'.($_SERVER['HTTP_HOST']).($_SERVER['REQUEST_URI']).'')); ?>&t=<?php print(urlencode($_title)); ?>" target="_blank" onclick="window.open(this.href, '_blank', 'status=0, height=400, width=360, resizable=0' ); return false;" style="float:left;margin-left:10px;"><img src="/pic/facebook.png" alt="Megosztás Facebookon" height="60" border="0" /></a>
				<a href="https://twitter.com/intent/tweet?url=<?php print(urlencode('https://'.($_SERVER['HTTP_HOST']).($_SERVER['REQUEST_URI']).'')); ?>&text=<?php print(urlencode($_title)); ?>" target="_blank" onclick="window.open(this.href, '_blank', 'status=0, height=550, width=360, resizable=0' ); return false;" style="float:left;margin-left:10px;"><img src="/pic/twitter.png" alt="Megosztás Twitteren" height="60" border="0" /></a>
        Partnereink:
        <a href="http://www.mascus.hu/Anyagmozgatas/Hasznalt-Targoncak" target="_blank">Használt targoncák</a> |
        <a href="http://www.topapro.hu/aprohirdetesek/epitoanyag-gep-szerszam/targonca/?oldal=1" target="_blank">Targonca hirdetések</a>
    </div>
  </div>
	<style>
		#szechenyi::before {
	...
	<section id="szechenyi">
			<section class="content">
				<a href="/vekop_1_2_6_20_2020_00740_c_fekvo_nyomdai_saxon2.pdf" target="_blank" title="PDF: Széchenyi 2020, Európai Uniós támogatás"></a>
			</section>
			<button class="close-container" onclick="szechenyiClose()" aria-label="A felugró ablak bezárása">
