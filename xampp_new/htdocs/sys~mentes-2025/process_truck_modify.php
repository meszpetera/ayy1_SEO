<?php
  ...
  if(isauth())
  {
		...
		if(substr(strtolower($_FILES['pdf']['name']),-4)=='.pdf') {
          ...
          $stmt = "SELECT `truck_id`,`truck_saxon-id`,`truck_depot`,`truck_sub-depot`,`truck_product_status` FROM trucks WHERE `truck_id` = ".$truckid;
          $result = mysqli_query($link, $stmt);
          $data = mysqli_fetch_assoc($result);
          $old_depot = $data["truck_depot"];
          $old_sdepot = $data["truck_sub-depot"];
          $saxxid = $data["truck_saxon-id"];
          $oldstatus = $data["truck_product_status"];
          ...
          $mode =  $_REQUEST['mode'];
          $truck_product_status = $_REQUEST['truck_product_status'];   
          ...
          if ($mode=='beszer' || $mode=='eladas'){
              $stmt = "UPDATE trucks SET `truck_product_status`=".((isSet($_REQUEST['truck_product_status']))?($_REQUEST['truck_product_status']):($truck_product_status)).",
                                         `truck_depot`         =".$depot.",
          ...
          if ($mode==''){
              $truck_product_status = $oldstatus;
              $stmt = "UPDATE trucks SET `truck_product_status`=".((isSet($_REQUEST['truck_product_status']))?($_REQUEST['truck_product_status']):($truck_product_status))." WHERE `truck_id` = ".$truckid;
              $result = mysqli_query($link, $stmt);
          }
