...
if (!loggedin() || !isauth()) {
    ...
    $stmt = $mysql->prepare($sql['admin_edit_make:list']);
...
//        $list .= '<td>' . '<input type="checkbox" name="ispart' . $type['ID'] . '" '. ($type['ispart'] == 0 ? "checked" : "") . '"/></td>';
    ...
    $main_content = $template->compile("sys/lang/hun/admin_edit_make");  //WARNING: Language hardcoded, no other languages needed    
